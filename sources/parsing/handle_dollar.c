...
/*   Updated: 2025/04/07 13:58:01 by tprovost         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int	put_char_to_str(t_data *data, char *cmd, char *str, int *i_n)
{
	...
	while (cmd[i_n[0]] != '\0' && n == 1)
	{
		...
		else if (cmd[i_n[0]] == '$' && cmd[i_n[0] + 1] != '\0' \
			&& !ft_isspace(cmd[i_n[0] + 1]) && !ft_isquote(cmd[i_n[0] + 1]))
			n = put_env_var_value(data, cmd, str, i_n);
		else if (cmd[i_n[0]] == '$' && ft_isquote(cmd[i_n[0] + 1]) == 1)
			i_n[0]++;
		else if (cmd[i_n[0]] != '\0')
		{
...
char	*handle_dollars(t_data *data, char *cmd)
{
	...
	while (cmd[i] != '\0')
	{
		if (cmd[i] == '\'')
			...
			&& ft_isspace(cmd[i + 1]) == 0 && ft_isquote(cmd[i + 1]) == 0)
			n = len_env_var_value(data, cmd, &i, n);
		else if (cmd[i] == '$' && ft_isquote(cmd[i + 1]) == 1)
			i++;
		else
		{
			n++;
